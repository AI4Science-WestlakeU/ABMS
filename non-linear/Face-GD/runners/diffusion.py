...
from functions.denoising import (arcface_ddim_diffusion, arcface_ddim_diffusion_dsg, arcface_ddim_diffusion_ours)
...
class Diffusion(object):
    ...
    def sample_sequence(self, model, cls_fn, mode, ref_path):
        ...
        for index in pbar:
            ...
            x, z = self.sample_image_alogrithm_arcface_ddim(x, model, last=False, cls_fn=cls_fn,
                                                            rho_scale=args.rho_scale, stop=args.stop, guidance_rate=args.guidance_rate, ref_path=ref_path,
                                                            method=args.method)
    ...
    def sample_image_alogrithm_arcface_ddim(self, x, model, last=True, cls_fn=None, rho_scale=1.0, stop=100, guidance_rate=0.05,
        ...
        if method == 'freedom':
            x = arcface_ddim_diffusion(x, seq, model, self.betas, cls_fn=cls_fn, rho_scale=rho_scale, stop=stop, guidance_rate=guidance_rate,
                                       ref_path=ref_path)
        elif method == 'dsg':
            x = arcface_ddim_diffusion_dsg(x, seq, model, self.betas, cls_fn=cls_fn, rho_scale=rho_scale, stop=stop, guidance_rate=guidance_rate,
                                            ref_path=ref_path)
            
        elif method == 'ours':
            x = arcface_ddim_diffusion_ours(x, seq, model, self.betas, cls_fn=cls_fn, rho_scale=rho_scale, stop=stop, guidance_rate=guidance_rate,
                                            ref_path=ref_path)
