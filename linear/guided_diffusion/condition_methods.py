...
class OurConditioningFull(ConditioningMethod):
    ...
    #         noise = torch.randn((self.n_guidance_directions, *x_t_mean.shape), device=x_t_mean.device)
    #         noise.requires_grad_(True)
    #         noise_mem = torch.cuda.memory_allocated()
    #         print(f"After noise generation: {(noise_mem - initial_mem) / 1024**2:.2f} MB")
            
    #         # Expand x_t_mean to match batch size
    #         x_t_mean_expanded = x_t_mean.expand(self.n_guidance_directions, -1, -1, -1, -1)
    #         x_ = x_t_mean_expanded + sigma_t * noise
    #         expand_mem = torch.cuda.memory_allocated()
    #         print(f"After expansion: {(expand_mem - initial_mem) / 1024**2:.2f} MB")
            
    #         # Reshape to 4D for UNet
    ...
    #         t = torch.tensor([t] * x_.shape[0], device=x_.device)
    #         x_0_hat = func(x_, t)
    #         func_mem = torch.cuda.memory_allocated()
    #         print(f"After UNet forward: {(func_mem - initial_mem) / 1024**2:.2f} MB")
            
    #         kwargs.pop('x_0_hat', None)
    #         norm, noise_grad = self.noise_grad(noise=noise, x_0_hat=x_0_hat, measurement=measurement, **kwargs)
    #         grad_mem = torch.cuda.memory_allocated()
    #         print(f"After gradient computation: {(grad_mem - initial_mem) / 1024**2:.2f} MB")
    ...
    #         # Compute norm in-place
    #         noise_grad_norm = torch.linalg.norm(noise_grad, dim=[1, 2, 3], keepdim=True)
    #         noise_grad_norm.add_(1e-20)  # Add epsilon in-place
            
    #         # Compute d_star in-place
    #         d_star = noise_grad.mul_(-r)  # Multiply by -r in-place
    #         d_star.div_(noise_grad_norm)  # Divide by norm in-place
            
    #         # Clear noise_grad as we don't need it anymore
    #         del noise_grad
    #         torch.cuda.empty_cache()
            
    #         with torch.no_grad():
    #             noise.add_(d_star)  # Add d_star in-place
    #             noise_mean = noise.mean(dim=0) 
                
    #         x_t_ = x_t_mean + sigma_t * noise_mean
            
    #         # Clear remaining tensors
    #         del noise, d_star
    #         torch.cuda.empty_cache()
            
    #         # Print final peak memory
