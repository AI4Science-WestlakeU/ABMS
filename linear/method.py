...
class DDPM(SpacedDiffusion):
    ...
    def projected_gradient_update(self, z, lr):
        # 计算原始梯度 (假设已通过反向传播获得 z.grad)
        raw_grad = z.grad.data

        # === 投影到均值约束的切空间 ===
        # 移除梯度中的均值分量 (确保更新后 z 的均值不变)
        grad_mean = torch.mean(raw_grad, dim=0, keepdim=True)
        projected_grad = raw_grad - grad_mean

        # === 投影到协方差约束的切空间 ===
        # 计算 z 与投影后梯度的协方差项并正交化
        cov = torch.mean(z * projected_grad, dim=0, keepdim=True)  # E[z * g_proj]
        projected_grad -= cov * z  # 正交化协方差扰动

        # === 执行投影梯度更新 ===
        z -= lr * projected_grad

        # === 协方差微调 (可选，用于数值稳定性) ===
        # 由于正交化可能存在数值误差，轻微调整协方差
        with torch.no_grad():
            z_mean = z.mean(dim=0, keepdim=True)
            z -= z_mean  # 确保均值严格为0
            z_std = z.std(dim=0, keepdim=True)
            z /= z_std  # 调整标准差为1 (不影响分布形态)

        return z

    def optimize_add_noise(self, x, t, tags, durations, classifier, char_ids, style_features, lr):
        # sample, sample_mean, sigma_t, pred_xstart
        ...
        for i in range(hp.time_step):
            ...
            '''if 15 < t < 300:
                ...
                with torch.enable_grad():
                    ...
                    ## DPS + DSG方法 此时不需要下面的添加噪声
                    # x = self.dsg(x, classifier, char_ids, 0.01, t, predict_noise)
            else:
                x = self.remove_noise(x, t, predict_noise)'''
            ...
            if t > 15:
                if t<0:
                    with torch.enable_grad():
                        ### guidance on noise ###
                        x = self.optimize_add_noise(x, t, tags, durations, classifier, char_ids, style_features, lr=10e6)
                else:
                    sigma = torch.sqrt(0.8 * self.beta[t] * (1 - self.alpha_cumprod[t-1]) / (1 - self.alpha_cumprod[t]))
